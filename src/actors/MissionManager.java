...
public class MissionManager extends Actor {
	...
	public MissionManager(HashMap<String, UDO> inputs, HashMap<String, UDO> outputs) {
		...
		RX_PS.addTransition(
				...
				RX_PS, null, 0);
		RX_PS.addTransition(
				new UDO[]{inputs.get(UDO.PS_END_MM), inputs.get(UDO.MM_TARGET_DESCRIPTION_MM), inputs.get(UDO.MM_NEW_SEARCH_MM)},
				new UDO[]{outputs.get(UDO.MM_TARGET_DESCRIPTION_MM), outputs.get(UDO.MM_NEW_SEARCH_MM)},
				POKE_OP, null, 0);
		POKE_OP.addTransition(
				new UDO[]{inputs.get(UDO.MM_TARGET_DESCRIPTION_MM), inputs.get(UDO.MM_NEW_SEARCH_MM)},
				new UDO[]{outputs.get(UDO.MM_POKE_OP), outputs.get(UDO.MM_TARGET_DESCRIPTION_MM), outputs.get(UDO.MM_NEW_SEARCH_MM)},
				POKE_OP, null, 0);
		POKE_OP.addTransition(
				new UDO[]{inputs.get(UDO.OP_ACK_MM), inputs.get(UDO.MM_TARGET_DESCRIPTION_MM), inputs.get(UDO.MM_NEW_SEARCH_MM)},

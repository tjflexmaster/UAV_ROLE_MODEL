...
public enum DATA_MM_MM_COMM{
	...
	MM_END_TRANSMIT_SEARCH_COMPLETE_MM,
	MM_END_SEARCH_MM,
	MM_END_LISTEN_TO_PS_MM,
	MM_START_TRANSMIT_AOI_MM,
	MM_START_SEARCH_MM,
	MM_START_TRANSMIT_TARGET_DESCRIPTION_MM,
	MM_START_TRANSMIT_TERMINATE_SEARCH_MM,
	MM_END_TRANSMIT_AOI_MM,
	MM_END_LISTEN_TO_OP_MM,
	MM_START_TRANSMIT_SEARCH_COMPLETE_MM,
	MM_END_TRANSMIT_TARGET_DESCRIPTION_MM,
	MM_END_LISTEN_TO_VO_MM,
	MM_END_VERIFICATION_MM,
}
...
public MissionManager(ComChannelList inputs, ComChannelList outputs) {
 ...
 public void initializeRX_PS(ComChannelList inputs, ComChannelList outputs, State RX_PS, State IDLE) {
	// (RX_PS,[A=PS_END_MM],[],1,NEXT,1.0)x(IDLE,[D=MM_END_LISTEN_TO_PS_MM],[])
	RX_PS.add(new Transition(_internal_vars, inputs, outputs, IDLE, Duration.NEXT.getRange(), 1, 1.0) {
		...
		public boolean isEnabled() { 
			...
			setTempOutput(Channels.DATA_MM_MM_COMM.name(), MissionManager.DATA_MM_MM_COMM.MM_END_LISTEN_TO_PS_MM);
			return true;
		}
	});
	// (RX_PS,[A=PS_NEW_SEARCH_AOI_MM],[],1,NEXT,1.0)x(IDLE,[D=MM_END_LISTEN_TO_PS_MM,D=MM_START_TRANSMIT_AOI_MM,D=MM_START_SEARCH_MM],[AREA_OF_INTEREST=NEW])
	RX_PS.add(new Transition(_internal_vars, inputs, outputs, IDLE, Duration.NEXT.getRange(), 1, 1.0) {
		@Override
		public boolean isEnabled() { 
	...
	// (RX_PS,[A=PS_TARGET_DESCRIPTION_MM],[],1,NEXT,1.0)x(IDLE,[D=MM_END_LISTEN_TO_PS_MM,D=MM_START_TRANSMIT_TARGET_DESCRIPTION_MM],[TARGET_DESCRIPTION=NEW])
	RX_PS.add(new Transition(_internal_vars, inputs, outputs, IDLE, Duration.NEXT.getRange(), 1, 1.0) {
		@Override
		public boolean isEnabled() { 
	...
	// (RX_PS,[A=PS_TERMINATE_SEARCH_MM],[],1,NEXT,1.0)x(IDLE,[D=MM_END_LISTEN_TO_PS_MM,D=MM_START_TRANSMIT_TERMINATE_SEARCH_MM],[TERMINATE_SEARCH_VO=NEW,TERMINATE_SEARCH_OP=NEW])
	RX_PS.add(new Transition(_internal_vars, inputs, outputs, IDLE, Duration.NEXT.getRange(), 1, 1.0) {
		@Override
		public boolean isEnabled() { 
 ...
 public void initializeTX_PS(ComChannelList inputs, ComChannelList outputs, State TX_PS, State END_PS) {
	// (TX_PS,[],[SEARCH_COMPLETE=TRUE],1,MM_TX_PS,1.0)x(END_PS,[A=MM_SEARCH_COMPLETE_PS,D=MM_END_TRANSMIT_SEARCH_COMPLETE_MM,D=MM_END_SEARCH_MM],[SEARCH_COMPLETE=FALSE])
	TX_PS.add(new Transition(_internal_vars, inputs, outputs, END_PS, Duration.MM_TX_PS.getRange(), 1, 1.0) {
		...
		public boolean isEnabled() { 
			...
			setTempOutput(Channels.DATA_MM_MM_COMM.name(), MissionManager.DATA_MM_MM_COMM.MM_END_TRANSMIT_SEARCH_COMPLETE_MM);
			setTempOutput(Channels.DATA_MM_MM_COMM.name(), MissionManager.DATA_MM_MM_COMM.MM_END_SEARCH_MM);
			setTempInternalVar("SEARCH_COMPLETE", false);
			return true;
		}
	});
	// (TX_PS,[],[SEARCH_FAILED=TRUE],1,MM_TX_PS,1.0)x(END_PS,[A=MM_SEARCH_FAILED_PS,D=MM_END_SEARCH_MM],[SEARCH_FAILED=FALSE])
	TX_PS.add(new Transition(_internal_vars, inputs, outputs, END_PS, Duration.MM_TX_PS.getRange(), 1, 1.0) {
		@Override
		public boolean isEnabled() { 
			if(!new Boolean(true).equals(_internal_vars.getVariable ("SEARCH_FAILED"))) {
	...
	// (TX_PS,[],[TARGET_SIGHTED_T=TRUE],1,MM_TX_PS,1.0)x(END_PS,[A=MM_TARGET_SIGHTED_T_PS,D=MM_END_SEARCH_MM],[TARGET_SIGHTED_T=FALSE])
	TX_PS.add(new Transition(_internal_vars, inputs, outputs, END_PS, Duration.MM_TX_PS.getRange(), 1, 1.0) {
		@Override
		public boolean isEnabled() { 
			if(!new Boolean(true).equals(_internal_vars.getVariable ("TARGET_SIGHTED_T"))) {
	...
	// (TX_PS,[],[TARGET_SIGHTED_F=TRUE],1,MM_TX_PS,1.0)x(END_PS,[A=MM_TARGET_SIGHTED_F_PS,D=MM_END_SEARCH_MM],[TARGET_SIGHTED_F=FALSE])
	TX_PS.add(new Transition(_internal_vars, inputs, outputs, END_PS, Duration.MM_TX_PS.getRange(), 1, 1.0) {
		@Override
		public boolean isEnabled() { 
			if(!new Boolean(true).equals(_internal_vars.getVariable ("TARGET_SIGHTED_F"))) {
 ...
 public void initializeOBSERVING_VGUI(ComChannelList inputs, ComChannelList outputs, State OBSERVING_VGUI, State IDLE) {
	// (OBSERVING_VGUI,[],[],1,MM_TO_IDLE,1.0)x(IDLE,[D=MM_END_VERIFICATION_MM],[])
	OBSERVING_VGUI.add(new Transition(_internal_vars, inputs, outputs, IDLE, Duration.MM_TO_IDLE.getRange(), 1, 1.0) {
		@Override
		public boolean isEnabled() { 
			setTempOutput(Channels.DATA_MM_MM_COMM.name(), MissionManager.DATA_MM_MM_COMM.MM_END_VERIFICATION_MM);
			return true;
		}
	});
 ...
 public void initializeRX_OP(ComChannelList inputs, ComChannelList outputs, State RX_OP, State IDLE) {
	// (RX_OP,[A=OP_SEARCH_COMPLETE_MM],[],1,NEXT,1.0)x(IDLE,[D=MM_END_LISTEN_TO_OP_MM,D=MM_START_TRANSMIT_SEARCH_COMPLETE_MM],[SEARCH_COMPLETE=TRUE])
	RX_OP.add(new Transition(_internal_vars, inputs, outputs, IDLE, Duration.NEXT.getRange(), 1, 1.0) {
		...
		public boolean isEnabled() { 
			...
			setTempOutput(Channels.DATA_MM_MM_COMM.name(), MissionManager.DATA_MM_MM_COMM.MM_END_LISTEN_TO_OP_MM);
	...
	// (RX_OP,[A=OP_SEARCH_FAILED_MM],[],1,NEXT,1.0)x(IDLE,[D=MM_END_LISTEN_TO_OP_MM],[SEARCH_FAILED=TRUE])
	RX_OP.add(new Transition(_internal_vars, inputs, outputs, IDLE, Duration.NEXT.getRange(), 1, 1.0) {
		@Override
		public boolean isEnabled() { 
 ...
 public void initializeTX_VGUI(ComChannelList inputs, ComChannelList outputs, State TX_VGUI, State END_VGUI) {
	// (TX_VGUI,[],[FLYBY_REQ_T=TRUE],1,MM_TX_VGUI,1.0)x(END_VGUI,[D=MM_FLYBY_REQ_T_VGUI,D=MM_END_VERIFICATION_MM],[FLYBY_REQ_T=FALSE])
	...
	// (TX_VGUI,[],[FLYBY_REQ_F=TRUE],1,MM_TX_VGUI,1.0)x(END_VGUI,[D=MM_FLYBY_REQ_F_VGUI,D=MM_END_VERIFICATION_MM],[FLYBY_REQ_F=FALSE])
	TX_VGUI.add(new Transition(_internal_vars, inputs, outputs, END_VGUI, Duration.MM_TX_VGUI.getRange(), 1, 1.0) {
		@Override
		public boolean isEnabled() { 
			if(!new Boolean(true).equals(_internal_vars.getVariable ("FLYBY_REQ_F"))) {
	...
	// (TX_VGUI,[],[ANOMALY_DISMISSED_T=TRUE],1,MM_TX_VGUI,1.0)x(END_VGUI,[D=MM_ANOMALY_DISMISSED_T_VGUI,D=MM_END_VERIFICATION_MM],[ANOMALY_DISMISSED_T=FALSE])
	TX_VGUI.add(new Transition(_internal_vars, inputs, outputs, END_VGUI, Duration.MM_TX_VGUI.getRange(), 1, 1.0) {
		@Override
		public boolean isEnabled() { 
			if(!new Boolean(true).equals(_internal_vars.getVariable ("ANOMALY_DISMISSED_T"))) {
	...
	// (TX_VGUI,[],[ANOMALY_DISMISSED_F=TRUE],1,MM_TX_VGUI,1.0)x(END_VGUI,[D=MM_ANOMALY_DISMISSED_F_VGUI,D=MM_END_VERIFICATION_MM],[ANOMALY_DISMISSED_F=FALSE])
	TX_VGUI.add(new Transition(_internal_vars, inputs, outputs, END_VGUI, Duration.MM_TX_VGUI.getRange(), 1, 1.0) {
		@Override
		public boolean isEnabled() { 
			if(!new Boolean(true).equals(_internal_vars.getVariable ("ANOMALY_DISMISSED_F"))) {
 ...
 public void initializeTX_OP(ComChannelList inputs, ComChannelList outputs, State TX_OP, State END_OP) {
	// (TX_OP,[],[AREA_OF_INTEREST=NEW],1,MM_TX_OP,1.0)x(END_OP,[A=MM_NEW_SEARCH_AOI_OP,D=MM_END_TRANSMIT_AOI_MM],[AREA_OF_INTEREST=CURRENT])
	TX_OP.add(new Transition(_internal_vars, inputs, outputs, END_OP, Duration.MM_TX_OP.getRange(), 1, 1.0) {
		...
		public boolean isEnabled() { 
			...
			setTempOutput(Channels.DATA_MM_MM_COMM.name(), MissionManager.DATA_MM_MM_COMM.MM_END_TRANSMIT_AOI_MM);
 ...
 public void initializeRX_VO(ComChannelList inputs, ComChannelList outputs, State RX_VO, State IDLE) {
	// (RX_VO,[A=VO_TARGET_SIGHTED_F_MM],[],1,NEXT,1.0)x(IDLE,[D=MM_END_LISTEN_TO_VO_MM],[TARGET_SIGHTED_F=TRUE])
	RX_VO.add(new Transition(_internal_vars, inputs, outputs, IDLE, Duration.NEXT.getRange(), 1, 1.0) {
		...
		public boolean isEnabled() { 
			...
			setTempOutput(Channels.DATA_MM_MM_COMM.name(), MissionManager.DATA_MM_MM_COMM.MM_END_LISTEN_TO_VO_MM);
			setTempInternalVar("TARGET_SIGHTED_F", true);
			return true;
		}
	});
	// (RX_VO,[A=VO_TARGET_SIGHTED_T_MM],[],1,NEXT,1.0)x(IDLE,[D=MM_END_LISTEN_TO_VO_MM],[TARGET_SIGHTED_T=TRUE])
	RX_VO.add(new Transition(_internal_vars, inputs, outputs, IDLE, Duration.NEXT.getRange(), 1, 1.0) {
		@Override
		public boolean isEnabled() { 
 ...
 public void initializeTX_VO(ComChannelList inputs, ComChannelList outputs, State TX_VO, State END_VO) {
	// (TX_VO,[],[TARGET_DESCRIPTION=NEW],1,MM_TX_VO,1.0)x(END_VO,[A=MM_TARGET_DESCRIPTION_VO,D=MM_END_TRANSMIT_TARGET_DESCRIPTION_MM],[TARGET_DESCRIPTION=CURRENT])
	TX_VO.add(new Transition(_internal_vars, inputs, outputs, END_VO, Duration.MM_TX_VO.getRange(), 1, 1.0) {
		...
		public boolean isEnabled() { 
			...
			setTempOutput(Channels.DATA_MM_MM_COMM.name(), MissionManager.DATA_MM_MM_COMM.MM_END_TRANSMIT_TARGET_DESCRIPTION_MM);

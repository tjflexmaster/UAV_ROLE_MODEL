...
public class Operator extends Actor {
	...
	public Operator(ComChannelList inputs, ComChannelList outputs) {
		...
		initializeOBSERVE_FLYBY(inputs, outputs, OBSERVE_FLYBY, IDLE);
	...
	private void initializeIDLE(ComChannelList inputs, ComChannelList outputs, State IDLE, State RX_MM, State LAUNCH_UAV, State OBSERVE_GUI) {
		...
		IDLE.add(new Transition(_internal_vars, inputs, outputs, OBSERVE_GUI){
			@Override
			public boolean isEnabled(){
				if(OperatorGui.VIDEO_OGUI_OP_DATA.UAV_FLYING_NORMAL.equals(_inputs.get(Channels.VIDEO_OGUI_OP_COMM))){
					return true;
				} else if (OperatorGui.VIDEO_OGUI_OP_DATA.UAV_FLYING_FLYBY.equals(_inputs.get(Channels.VIDEO_OGUI_OP_COMM))){
	...
	private void initializeOBSERVE_GUI(ComChannelList inputs, ComChannelList outputs, State OBSERVE_GUI, State POKE_OGUI, State POST_FLIGHT, State OBSERVE_UAV) {
		...
		OBSERVE_GUI.add(new Transition(_internal_vars, inputs, outputs, POKE_OGUI, Duration.NEXT.getRange(),1){
			@Override
			public boolean isEnabled(){
				if(_inputs.get("VIDEO_OGUI_OP_COMM").equals(OperatorGui.VIDEO_OGUI_OP_DATA.OGUI_FLYBY_REQ_F)){
					this.setTempOutput("VISUAL_OP_OGUI_COMM", Operator.VISUAL_OP_OGUI_COMM.OP_POKE_OGUI);
					return true;
				}else if(_inputs.get("VIDEO_OGUI_OP_COMM").equals(OperatorGui.VIDEO_OGUI_OP_DATA.OGUI_FLYBY_REQ_T)){
					this.setTempOutput("VISUAL_OP_OGUI_COMM", Operator.VISUAL_OP_OGUI_COMM.OP_POKE_OGUI);
					return true;
				}
				return false;
		...
		OBSERVE_GUI.add(new Transition(_internal_vars, inputs, outputs, OBSERVE_UAV,Duration.NEXT.getRange(),1){
			@Override
			public boolean isEnabled(){
				if(_inputs.get("VIDEO_OGUI_OP_COMM").equals(OperatorGui.VIDEO_OGUI_OP_DATA.OGUI_LANDED)){
					return true;
				}
				return false;
			}
	...
	private void initializeOBSERVE_UAV(ComChannelList inputs, ComChannelList outputs, State OBSERVE_UAV, State POST_FLIGHT, State OBSERVE_GUI) {
		...
		//(OBSERVE_UAV,[],[])x(OBSERVE_GUI,[],[])
		OBSERVE_UAV.add(new Transition(_internal_vars, inputs, outputs, OBSERVE_GUI, Duration.NEXT.getRange(), 1){
			@Override
			public boolean isEnabled(){
				return true;
			}

...
public enum DATA_PS_PS_COMM{
	PS_END_LISTEN_TO_MM_PS,
	PS_END_TRANSMIT_AOI_PS,
	PS_END_TRANSMIT_TARGET_DESCRIPTION_PS,
...
public ParentSearch(ComChannelList inputs, ComChannelList outputs) {
 ...
 public void initializeIDLE(ComChannelList inputs, ComChannelList outputs, State RX_MM, State IDLE, State POKE_MM) {
	// (IDLE,[E=NEW_SEARCH_EVENT],[],1,NEXT,1.0)x(POKE_MM,[A=PS_POKE_MM],[SEARCH_ACTIVE=TRUE,NEW_SEARCH_AOI=++,NEW_TARGET_DESCRIPTION=++])
	...
	// (IDLE,[E=NEW_SEARCH_AREA_EVENT],[SEARCH_ACTIVE=FALSE],1,NEXT,1.0)x(POKE_MM,[D=PS_START_TRANSMIT_AOI_PS],[SEARCH_ACTIVE=TRUE,NEW_SEARCH_AOI=++])
	IDLE.add(new Transition(_internal_vars, inputs, outputs, POKE_MM, Duration.NEXT.getRange(), 1, 1.0) {
		@Override
		public boolean isEnabled() { 
	...
	// (IDLE,[E=TARGET_DESCRIPTION_EVENT],[SEARCH_ACTIVE=FALSE],1,NEXT,1.0)x(POKE_MM,[A=PS_POKE_MM],[SEARCH_ACTIVE=TRUE,NEW_TARGET_DESCRIPTION=++])
	IDLE.add(new Transition(_internal_vars, inputs, outputs, POKE_MM, Duration.NEXT.getRange(), 1, 1.0) {
		@Override
		public boolean isEnabled() { 
 ...
 public void initializeTX_MM(ComChannelList inputs, ComChannelList outputs, State TX_MM, State END_MM) {
	// (TX_MM,[],[NEW_SEARCH_AOI>0],1,PS_TX_MM,1.0)x(END_MM,[A=PS_NEW_SEARCH_AOI_MM,D=PS_END_TRANSMIT_AOI_PS],[NEW_SEARCH_AOI=--])
	TX_MM.add(new Transition(_internal_vars, inputs, outputs, END_MM, Duration.PS_TX_MM.getRange(), 1, 1.0) {
		...
		public boolean isEnabled() { 
			...
			setTempOutput(Channels.DATA_PS_PS_COMM.name(), ParentSearch.DATA_PS_PS_COMM.PS_END_TRANSMIT_AOI_PS);
			setTempInternalVar("NEW_SEARCH_AOI", (Integer)_internal_vars.getVariable("NEW_SEARCH_AOI") - 1);
			return true;
		}
	});
	// (TX_MM,[],[NEW_TARGET_DESCRIPTION>0],1,PS_TX_MM,1.0)x(END_MM,[A=PS_TARGET_DESCRIPTION_MM,D=PS_END_TRANSMIT_TARGET_DESCRIPTION_PS],[NEW_TARGET_DESCRIPTION=--])
	TX_MM.add(new Transition(_internal_vars, inputs, outputs, END_MM, Duration.PS_TX_MM.getRange(), 1, 1.0) {
		@Override
		public boolean isEnabled() { 
			if(_internal_vars.getVariable("NEW_TARGET_DESCRIPTION") instanceof Integer && new Integer(0) >= (Integer) _internal_vars.getVariable ("NEW_TARGET_DESCRIPTION")) {
			...
			setTempOutput(Channels.DATA_PS_PS_COMM.name(), ParentSearch.DATA_PS_PS_COMM.PS_END_TRANSMIT_TARGET_DESCRIPTION_PS);
			setTempInternalVar("NEW_TARGET_DESCRIPTION", (Integer)_internal_vars.getVariable("NEW_TARGET_DESCRIPTION") - 1);
			return true;
		}
	});
 ...
 public void initializeRX_MM(ComChannelList inputs, ComChannelList outputs, State RX_MM, State IDLE) {
	// (RX_MM,[A=MM_SEARCH_COMPLETE_PS],[],1,NEXT,1.0)x(IDLE,[D=PS_END_LISTEN_TO_MM_PS],[SEARCH_COMPLETE=TRUE,SEARCH_ACTIVE=FALSE])
	RX_MM.add(new Transition(_internal_vars, inputs, outputs, IDLE, Duration.NEXT.getRange(), 1, 1.0) {
		...
		public boolean isEnabled() { 
			...
			setTempOutput(Channels.DATA_PS_PS_COMM.name(), ParentSearch.DATA_PS_PS_COMM.PS_END_LISTEN_TO_MM_PS);
			setTempInternalVar("SEARCH_COMPLETE", true);
			setTempInternalVar("SEARCH_ACTIVE", false);
			return true;
		}
	});
	// (RX_MM,[A=MM_SEARCH_FAILED_PS],[],1,NEXT,1.0)x(IDLE,[D=PS_END_LISTEN_TO_MM_PS],[SEARCH_FAILED=TRUE,SEARCH_ACTIVE=FALSE])
	RX_MM.add(new Transition(_internal_vars, inputs, outputs, IDLE, Duration.NEXT.getRange(), 1, 1.0) {
		@Override
		public boolean isEnabled() { 
	...
	// (RX_MM,[A=MM_TARGET_SIGHTED_T_PS],[],1,NEXT,1.0)x(IDLE,[D=PS_END_LISTEN_TO_MM_PS],[TARGET_FOUND=TRUE])
	RX_MM.add(new Transition(_internal_vars, inputs, outputs, IDLE, Duration.NEXT.getRange(), 1, 1.0) {
		@Override
		public boolean isEnabled() { 
	...
	// (RX_MM,[A=MM_TARGET_SIGHTED_F_PS],[],1,NEXT,1.0)x(IDLE,[D=PS_END_LISTEN_TO_MM_PS],[TARGET_FOUND=TRUE])
	RX_MM.add(new Transition(_internal_vars, inputs, outputs, IDLE, Duration.NEXT.getRange(), 1, 1.0) {
		@Override
		public boolean isEnabled() { 
